...
For example, by representing the module of the function {% raw %}$$f(z) = \dfrac{1}{z^2+4}$${% endraw %}, where \\(z \in \mathbb{C}\\), we can detect two singularities, in this case two simple poles one at \\(z=-2\\) and the other at \\(z=2\\).
<br>
